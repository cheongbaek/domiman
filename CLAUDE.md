...
- **낚싯대**: 아래 두 조건의 **OR**.
  ...
  1. **최소 획득 시간이 정확히 `1`(초)** — 낚싯대 교체가 필요하면 게임이 최소
  ...
  2. **`cur + 5 >= mx`** (`_tank_needs_collect`) — 살림망이 최대치 5개 이내.
     **'살림망이 가득 차 낚시를 진행할 수 없습니다' 오류가 뜨는 상황을 평시에
     미리 막기 위한 것.** 교체한 낚싯대의 최대치가 더 크면(465 → 465/920)
     회수조차 필요 없이 낚시가 그대로 이어진다.

  **② 는 반드시 회수 분기보다 먼저 검사한다(함정):** ②와 회수 조건은 같은
  식이라, 순서가 뒤면 회수 분기의 `continue`가 삼켜 낚싯대 검사에 **도달조차
  못 한다.** 실제로 이 순서 때문에 `570/470`에서 교체가 안 돌았다.
  현재 순서: **낚싯대 → 회수 → 미끼 → 낚시정지 감지.**
  (회수 분기는 '낚싯대 자동교체' 체크박스가 꺼져 있을 때의 경로로 남는다.)
...
읽기 전 1초 대기는 교체한 낚싯대 기준으로 수량 표시가 갱신될 시간. 보고 순서는
...
| ESC **3번** | 교체 루틴 진입, `_resume_fishing` | 어떤 팝업이 떠 있을지 모르니 **무조건 정리**. 종류 불문 |
...
| `minsec == 1` | 낚싯대 트리거 ① | 교체 필요 시 게임이 '1초'로 표시. 낚싯대 **전용** 신호라는 점이 핵심 |
...
**미확인 — 바꾸기 전 반드시 물어볼 것** (아직 근거를 기록하지 못한 값):
`retries=3, delay=0.2`(`read_min_gain_time`) · `0 < sec <= 600`(최소시간 상한) ·
`max(3.0, last_interval)`(폴링 하한) · `last_interval = 20.0`(최초 간격) ·
`best_diff < 80.0`(윤곽선 매칭 임계) · `max_retries = 10`(퀴즈 재시도) ·
`range(4)`(낚시 취소 전 Enter+ESC 반복) · `time.sleep(1.0)`
(`_restart_or_collect_after_rod_swap`의 갱신 대기) · `w*0.15`(실루엣 여백 잘라내기) ·
`abs(cy-행Y) > 30`(카드 행 스냅 허용) · `fail_streak`(세기만 하고 임계 동작 없음) ·
클릭 `delay` 값들(1.0/1.2/1.5/2.0/2.5) · 루틴 후 `wait(2.0)` vs `wait(3.0)`
