...
`read_tank_quantity(allow_overflow=True)`로 수량을 한 번 읽고 갈린다:
...
`read_tank_quantity`의 `cur <= mx` 가드가 OCR 오인식 필터라 **초과 수량은
평소엔 파싱 자체가 버려진다** — 그래서 `allow_overflow=True`(mx의 3배까지
허용) 인자를 이 호출에서만 켠다. 판정 기준은 감시 루프와 공유
(`_tank_needs_collect` / `TANK_COLLECT_MARGIN = 5`). 읽기 전 1초 대기는
교체한 낚싯대 기준으로 화면 수량 표시가 갱신될 시간(교체 전 최대치를 그대로
읽어버리면 위 분기가 무의미해짐). 보고 순서는 `rs` → `y,r`/`x,r` →
(회수 시) `s` → `g`/`f`.
