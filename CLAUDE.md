...
**GUI '실시간 수량확인' 버튼에도 동일 적용, 단 `_query_tank`는 안 씀
(함정 — 캡처 소유권 경쟁):** `on_tank_check`는 수량 캐시 지름길이 있는
공용 `_query_tank`를 거치지 않고 **자체 배경 스레드 하나**에서 창 호출 →
캡처 확보 → 수량 읽기 → `is_fishing_active()` 확인까지 전부 끝낸다.
처음엔 `_query_tank(report)`로 수량을 받고 그 콜백 안에서 별도 스레드로
낚시 상태를 확인했는데, `_query_tank`도 자기 콜백 실행 **직후** finally에서
`game_capture.stop()`을 호출해 — 새로 띄운 확인 스레드가 막 캡처를
켜려는 순간과 경합(TOCTOU)해 캡처가 죽은 채로 판독을 시도, 조용히
`None`만 반환하고 재개 버튼을 누르지 못하는 사례가 있었다. 한 스레드
안에서 끝까지 처리하면 이 경합이 사라진다(원격 N 질의는 `_query_tank`를
그대로 씀 — 그쪽은 상태 확인이 필요 없어 캐시 지름길이 여전히 유용).
