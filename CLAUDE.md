...
**OCR 위치 (`REGION_FISHING_BTN=(1047,988,74,31)`, `낚시상태버튼OCR검증.py`로
실측):** 낚시 중인 라이브 화면에서 "'낚시 취소'"(conf 0.92, 앞뒤에 따옴표가
붙어 나옴 — 부분매칭이라 무관), 낚시 대기 중 스크린샷에서 '낚시 시작'
(conf 0.999)이 **거의 같은 좌표**에서 잡혀 "두 버튼 위치가 같다"는 전제를
확인했다. `is_fishing_active()`: '취소' 포함→True(진행 중), '시작' 포함→
False(대기 중), 둘 다 없으면 None(호출측이 무시).
...
**GUI '실시간 수량확인' 버튼에도 동일 적용:** 로컬 클릭 시(`on_tank_check`)
수량 보고 콜백 뒤에 `_check_and_resume_fishing()`을 이어서 호출 —
캐시 히트/미스와 무관하게 항상 새로 창을 앞으로 불러 캡처를 보장한 뒤
`is_fishing_active()`를 확인하고 필요하면 재개한다(원격 N 질의 경로는
건드리지 않음 — 그쪽은 다른 PC가 이 PC의 수량만 물어보는 용도).
