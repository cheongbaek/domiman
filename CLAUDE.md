...
- **낚싯대**: 아래 두 조건의 **OR**.
  ...
  1. **최소·최대 획득 시간이 둘 다 정확히 `1`(초)** — 낚싯대 교체가 필요하면
     ...
     **최소만 보면 안 된다 — easyocr이 '11초'를 '1초'로 읽는다(실측, 260813a):**
     ...
     넓혀도 그대로다(잘림이 아니라 인식 오류). 확대 배율로도 못 고친다(x2·x3는
     '11초', x4는 다시 '1초' — 안정적이지 않음).
     ...
     **한 번 읽고 갈면 안 된다 — `_confirm_rod_expiry` (260815b, 실측 사고):**
     ...
     그래서 교체 전에 두 단계를 더 둔다:
     ① 0.4초 뒤 **다시 읽어** 최소·최대가 계속 1초인지 확인(`ROD_EXPIRY_CONFIRM_READS=2`).
        한 프레임짜리 오인식은 여기서 걸러진다.
     ② 그래도 낚시가 **진행 중**('낚시 취소')으로 보이면 만료가 아니라고 본다.
        단 그 판독도 틀릴 수 있으니, **살림망 수량이 오래 멈춰 있으면**
        (`ROD_EXPIRY_FROZEN_CYCLES=10` ≈ 30초) 진행 중 표시를 무시하고 교체한다 —
        진짜 만료는 수량이 영원히 늘지 않으므로 이 구멍으로 반드시 빠져나온다.
...
| GUI | 'ntfy/SMS에 표시될 이름' 자리 → **[로그인] 버튼**, 그 옆 칸 → 접속 IP 표시 / 'ntfy 채널 이름' 자리 → **[로그아웃] 버튼**, 그 옆 칸 → 로그인 ID 표시 / 'ntfy 메시지' → **'domichat 메시지'** |
...
| `ROD_EXPIRY_FROZEN_CYCLES = 10` | 같은 곳 | 낚시가 '진행 중'으로 보여도 수량이 이만큼(≈30초) 멈춰 있으면 교체를 허용하는 **탈출구**. 없애면 상태 판독이 틀렸을 때 영영 교체를 못 한다 |
...
| `OCR_NUM_SCALE = 3` + 흑백 | `_prep_number_ocr` | 10가지 전처리를 4단계 열화에 돌려 고른 값(x3+흑백 7/12 > 그대로 6 > x4 4). **숫자 화이트리스트는 넣지 않는다 — 측정에서 오히려 나빠졌다**(3→2/9) |
