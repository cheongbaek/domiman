...
- **`cur > mx`(예: 520/470)는 '비정상'이 아니다** — 최대치가 더 작은 낚싯대로
  ...
  회수 조건에 걸려 바로 회수돼야 한다. 위 교체 섹션의 260809c 항목 참고.
...
**낚싯대 교체 직후 살림망 확인 (`_restart_or_collect_after_rod_swap`,
260809a에서 추가 — 함정):** 낚싯대마다 **최대 살림망 개수가 다르다.** 새로
든 낚싯대의 최대치가 기존보다 작으면 이미 쌓인 물고기가 그 최대치를 넘어
(예: `535/470`) **낚시가 아예 시작되지 않는다** — 예전엔 교체 후 무조건
'낚시 시작'을 눌렀기 때문에, 이 상태에 빠지면 클릭이 먹히지 않은 채
수량만 정체됐다. 이제 교체 직후 '낚시 시작'을 누르기 **직전에**
`read_tank_quantity()`로 수량을 한 번 읽고 갈린다:
- 여유 있음(예: `165/470`) → 평소대로 `COORD_FISHING_BTN` 클릭해 재개.
- 회수 조건(예: `467/470`)이거나 최대치 초과(예: `535/470`) →
  '낚시 시작'을 **누르지 않고** 곧바로 `run_fishing_routine()`. 회수 루틴은
  맨 앞에서 `is_fishing_active()`로 버튼 글자를 읽어 '낚시 시작'(대기 중)이면
  취소 절차를 스스로 생략하므로, 별도 감시 조건 없이 바로 불러도 안전하다.
- 판독 실패 → 평소 동작(그냥 시작)으로 폴백. 정말 초과 상태였다면 수량이
  정체되므로 감시 루프의 낚시 정지 감지가 결국 받는다.

판정 기준은 감시 루프와 공유(`_tank_needs_collect` / `TANK_COLLECT_MARGIN = 5`).
읽기 전 1초 대기는 교체한 낚싯대 기준으로 화면 수량 표시가 갱신될 시간(교체 전
최대치를 그대로 읽어버리면 위 분기가 무의미해짐). 보고 순서는 `rs` →
`y,r`/`x,r` → (회수 시) `s` → `g`/`f`.

**초과 수량 파싱은 전역이어야 한다 (260809c 수정 — 260809a의 반쪽 수정이 남긴
버그):** 260809a에서는 `read_tank_quantity`에 `allow_overflow` 인자를 새로
넣고 **이 교체 직후 호출에서만** 켰다. 잘못이었다. 초과는 교체 직후 한순간의
값이 아니라 **회수할 때까지 계속 남는 상태**이고, 그동안 감시 루프는 기본값
(초과 거부)으로 읽으니 매 사이클 `None` → '수량 파싱 실패'만 찍었다. 게다가
초과 상태에선 낚시가 안 걸려 수량이 줄지도 않으므로 **스스로 못 빠져나오는
영구 정체**가 된다(회수 조건 판정까지 도달하지 못하니 회수도 안 됨).
지금은 `allow_overflow` 인자를 없애고 **`read_tank_quantity`가 언제나
`cur > mx`를 그대로 돌려준다.** 이 덕분에 교체 루틴이 초과를 놓쳐도(예: 갱신
전 최대치를 읽어 '여유 있음'으로 판단) **감시 루프가 다음 사이클에 스스로
회수해 복구한다.**

**값 범위로 오인식을 거르는 가드는 두지 말 것(이 버그의 교훈):** 대체
가드로 `cur <= mx * 3` 상한을 잠깐 뒀다가 뺐다. 잡는 범위가 원리적이지 않다 —
`520/47`(max 절삭)은 우연히 걸리지만 앞자리가 빠진 `52/470`은 그냥 통과하니,
실효는 없고 "예상 못 한 정상 값을 조용히 버릴 위험"만 `cur <= mx`와 똑같이
남는다. 오인식은 `read_tank_quantity`의 프레임 4회 재시도와 감시 루프의 매
사이클 재파싱으로 씻겨 나가므로 그쪽에 맡긴다. 남은 조건은 `mx > 0` 하나이고,
이건 휴리스틱이 아니라 구조적 무의미값 차단이다(max가 0이면 `cur >= mx-5`가
항상 참이라 회수가 무한 반복).

판독 실패 시 원인 추적을 위해 마지막 OCR 원문을 전역 `_last_tank_ocr`에
남기고, 감시 루프의 기존 '수량 파싱 실패' 로그 한 줄에 함께 찍는다
(`is_fishing_active()`의 실패 로그와 같은 방식 — 줄 수는 늘지 않음).

**공통 마무리 (`_use_card` / `_use_card_and_restart`):** '사용하기' 클릭 →
1초 후 **ESC 2번(0.5초 간격)** 까지가 `_use_card`, 거기에 '낚시 시작' =
