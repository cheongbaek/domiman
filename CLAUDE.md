  ...
  1. **최소·최대 획득 시간이 둘 다 정확히 `1`(초)** — 낚싯대 교체가 필요하면
     ...
     **한 번 읽고 갈면 안 된다 — `_confirm_rod_expiry` (260815b, 실측 사고):**
     ...
     ~~③ 수량이 오래 멈춰 있으면 진행 중 표시를 무시하고 교체~~
        ...
        우회하는 구멍이었다. 버튼이 영구히 안 읽히는 사태의 통로는 남아 있다:
        아래 트리거 ②(`cur > mx`)는 버튼을 보지 않는다.
...
| 버튼 = '낚시 시작' **AND** | 낚싯대·미끼 교체 (260822a) | 교체가 필요한 상황이면 **낚시가 이미 멈춰 있다.** 값 판독과 원인이 다른 신호라, 기록에 남은 헛교체 사고 두 건이 이 조건 하나로 막힌다. **`None`(판독 불가)은 통과시키지 말 것** |
