...
`_reassert()`가 0.25초마다 `SetWindowPos(HWND_TOPMOST, …, SWP_NOACTIVATE)`로
최상위를 다시 주장한다 — `bring_game_to_front`가 게임을 앞으로 불러도 암막이
위에 남게. 클릭은 `WS_EX_TRANSPARENT` 덕에 그대로 게임으로 통과한다
(`WindowFromPoint`가 게임 창을 돌려주는 것으로 확인).
