...
# (개/켓/것/캠). 반면 **'금'은 5번 전부 살아남았다.**
#
# 260909a에 보유 미끼 **33종 전체(5페이지)** 를 전처리 5가지로 읽어 오인식을
# 전수 수집하고(아래 목록) 그것들을 OR로 넓혔다. 목표 칸의 실제 판독은
#   '항금캠지렇이' · '활금것지렇미' · 'n금캠지렇미'(x2) · '창급것지꼬이'
# 로, '금'조차 '급'으로 깨지고 꼬리가 '지꼬'까지 간다. 세 갈래를 OR로 두어
# **어느 한 조각이 깨져도 다른 갈래가 받는다**(실측 5/5 · 오검출 0):
#   ① [금급].?지[렁렇령런랑]      — '금/급' + 0~1글자 + '지렁'꼴 (주력)
#   ② [금급][캠켓것개캣컷][지찌]   — 꼬리가 '지꼬'처럼 깨진 경우
#   ③ [활왕항팔창황][금급]        — '황금' 두 글자만으로 (꼬리 판독 불필요)
#
# **'갯지렁'류를 OR에 넣지 말 것(260909a 실측, 함정):** 보유 목록에는
# '갯지렁이 훈제 구이'(1페이지)와 '다이아 갯지렁이'(4페이지)가 **따로 있어**,
# 옛 패턴 `[갯개]지[렁렇령]`을 쓰면 이 둘이 프로덕션 경로에서 4/5·4/5로
# 걸린다. 미끼 교체는 페이지를 순서대로 훑어 **처음 걸린 칸을 쓰므로**
# 1페이지의 훈제구이를 집어 조용히 엉뚱한 미끼를 쓰게 된다.
# 같은 이유로 `금.{0,2}`처럼 사이를 2글자까지 벌리는 것도 되돌리지 말 것 —
# '레볼루션 지렁이'가 '허금루선지렇미'로 읽혀 걸린다(사이를 1글자로 조이면
# 목표 적중은 그대로면서 이 오검출이 사라진다).
BAIT_TARGET_PATTERN = (r"[금급].?지[렁렇령런랑]"
                       r"|[금급][캠켓것개캣컷][지찌]"
                       r"|[활왕항팔창황][금급]")
...
#   매직 -> 매직 · 미적            (앞 두 글자가 통째로 깨진다)
#   스타 -> 스타                   (12/12, 한 번도 깨지지 않은 축)
#   장미 -> 장미 · 잠미
#   푸른 -> 푸른·푸튼·푸름·무릎·무른·꾸슨·꾸촌  (5/12만 생존 — 가장 약하다)
#   검   -> 경·검·겉·거            (그래서 '검'은 축으로 쓰지 않는다)
# **단음절 OR로 되돌리지 말 것:** 옛 패턴의 `푸` 한 글자는 '푸른'이 '무/꾸'로
# 깨지면 무력하면서, 앞으로 '푸'가 들어간 다른 낚싯대가 생기면 그대로
# 오검출이 된다. 두 글자 묶음이 더 넓고 동시에 더 안전하다.
# **'낚싯대'(낚시대/낚셋대/낚컷대/낚짓대)는 절대 축으로 쓰지 말 것** —
# 모든 낚싯대에 들어 있어 아무 칸이나 걸린다.
ROD_TARGET_PATTERN = (r"스타"
                      r"|[장잠][미며]"
                      r"|[매메미민][직적졕]"
                      r"|[푸무꾸][른튼름슨촌릎]")
...
def press_esc(delay=0.5):
    """ESC 전송. 간격은 **0.5초로 통일**(260909a) — click_real·press_key의
    기본값과 같다. 호출부에서 delay를 따로 넘기지 말고 이 기본값을 쓸 것."""
...
def _find_cards_by_pattern(pattern):
    ...
    **크롭 배율로 나눠야 한다(260909a, 실제 증상으로 고침 — 되돌리지 말 것):**
    `_watch_grab_region`은 260815c부터 1080p로 축소한 프레임이 아니라 **WGC
    원본 프레임에서** 자르므로, QHD에서는 크롭이 FHD 영역보다 크게 나온다
    (실측: 게임 클라이언트 2400x1350 -> 크롭 801x253, 배율 1.252). 그 크롭
    픽셀을 FHD 오프셋에 그대로 더하던 예전 코드는 아래 두 증상을 냈다:
      · **2행이 통째로 버려진다** — 행 오차가 (660-483)x0.252 = 44px로 아래
        `> 30` 가드에 걸린다. 목표가 2행에 있으면(매직 스타·푸른 장미검·황금
        갯지렁이 모두 2행) 영구 미발견 -> 좌상단 폴백으로 **엉뚱한 카드 사용**.
      · **1행도 열이 밀린다** — 3열이 4열로 잡혀 옆 칸의 '사용하기'를 누른다.
    FHD(배율 1.0)에서는 결과가 예전과 글자 그대로 같아, 검증된 좌표는 그대로
    유효하다. 라이브 검증: 보정 전 3/3 미발견 -> 보정 후 3/3 정확."""
    x0, y0, rw, rh = REGION_BAIT_NAMES
    for _ in range(2):
        img = _watch_grab_region(REGION_BAIT_NAMES)
        results = reader.readtext(img) if img is not None else []
        ch, cw = img.shape[:2] if img is not None else (rh, rw)
        found = []
        for box, text, _conf in results:
            ntext = text.replace(" ", "")
            if not re.search(pattern, ntext):
                continue
            cx = x0 + (sum(p[0] for p in box) / 4.0) * rw / float(cw)
            cy = y0 + (sum(p[1] for p in box) / 4.0) * rh / float(ch)
