...
> **지금 상태: 웹앱만 배포돼 있고 중계(서버 [8절])는 아직 채팅을 모른다.**
> 그래서 브라우저에는 '채팅방 목록' 칸이 **뜨지 않는다** — `ready` 에 `chat: true`
> 가 실려야 나타난다. 중계 쪽 구현은 통합 전 `domiweb.py` 에서 완성·검증했고
> (아래 260913a 기록), `domiserver.py` [8절]로 옮겨 넣는 일만 남았다. 서버에
> 들어가는 순간 배포된 웹앱이 그대로 받아 쓴다(웹앱은 다시 배포할 필요가 없다).
...
## 검증 기록 (260913a — 채팅, 통합 **전** domiweb.py 기준)

중계 쪽 구현은 아직 `domiserver.py` 에 들어가지 않았다(위 상태 문단 참고). 아래는
그 구현을 옮겨 넣을 때 그대로 다시 돌려야 할 항목이다.

격리 서버 사본(평문 47899) + 중계 + 브라우저 흉내(RFC 6455 최소 구현)로 **22항목**:
...
웹앱은 실제 Chromium으로 **28항목**(휴대폰 폭 430px + PC 1280px, 라이트/다크):
...
`←`↔[나가기] 교체). 더해 **통합된 중계(채팅 미지원)에 붙여 4항목** — 붙는 것,
채팅 칸이 뜨지 않는 것, 낚시 제어·수량 방송이 그대로인 것.

**이 과정에서 고친 것:**
- 상단 바를 **접히게**(`flex-wrap`) 했다 — 칸이 하나 늘면서 430px에서 연결 상태
  글자가 세 줄로 쪼개지고 살림망 칸이 화면 밖으로 잘렸다.
- 이미지 버튼은 **글자('사진')** 다 — 그림 문자(🖼)는 글꼴에 따라 두부(□)로 나온다.
- 옛 중계에서 `_ensure_join` 에 **5초 중복 방지**를 넣었다(같은 방에 두 번 들어가
  `S` 질의가 두 번 나갔다). 통합본은 방 존재를 DB로 바로 보므로 그 자리가 다르다 —
  옮길 때 같은 증상이 나는지 확인할 것.
