...
| 파일 | 역할 |
|---|---|
| `domiserver.py` [8. 웹 중계] | 중계를 **서버 안에서** 돌리는 길(260914a 통합) |
| `domiweb.py` | 중계를 **따로** 돌리는 길 — `web` 계정으로 127.0.0.1:47821 에 붙는다(260914d 복원) |
...
## 중계를 어디서 돌릴까 — 길이 둘이다 (둘 중 하나만)

| | 서버 안(`domiserver.py` [8]) | 따로(`domiweb.py`) |
|---|---|---|
| 띄우는 법 | `domiserver.json` 의 `"web": true`(기본) | 서버는 `"web": false`, 그 다음 `python domiweb.py` |
| 서버와의 연결 | 가상 연결(`HubConn`) — 소켓이 없다 | **`web` 계정으로 127.0.0.1:47821 에 진짜로 붙는다**(TLS+지문 고정) |
| 계정 | 이름만 `ONLINE` 에 올린다(비밀번호 없음) | domichat 계정 하나가 필요(서버 콘솔에서 `approve web`) |
| 채팅 끄기 | `web_chat: false` | `chat: false` |
| 장점 | 프로그램 하나, 왕복 없음 | 서버에 웹 전용 상태가 섞이지 않고, 중계만 따로 껐다 켤 수 있다. 자격·지문이 바깥 클라이언트와 **똑같은 길**을 탄다 |

**동시에 띄우지 말 것** — 47822 를 두 번 열 수 없고, 같은 ID 로 두 번 로그인할 수도
없다(`already_online`). 브라우저 ↔ 중계 규격은 두 길이 **글자 하나 다르지 않다**.
웹앱은 어느 쪽에 붙었는지 모르고, 알 필요도 없다.
...
## 검증 기록 (260914d — 분리판 복원)

지금의 `domiserver.py`(내장 중계 `web: false` 로 끔) + 되살린 `domiweb.py` 로
**11항목**: 내장 중계가 실제로 안 뜨는 것, 중계가 `web` 계정으로 127.0.0.1 에
로그인하는 것, 방 목록(제어용 방 감춤), 공개방 입장·발신(cid 에코)·수신,
이미지 양방향(sha256·바이트 일치), 비밀번호 방 입장, 제어용 방 거절, 그리고
**낚시 제어 회귀**(입장·`S` 질의·상태 응답).
