...
실제 Chromium으로 **28항목**(휴대폰 폭 430px + PC 1280px, 라이트/다크): 상단 칸 →
목록 → 비번 창(오답·정답) → 채팅방 → 발신(✓ 표시) → 수신 → 이미지 받기/보내기 →
크게 보기 → 목록 → 낚시 화면 복귀(살림망 표시 유지). 폭별 배치도 함께 확인했다 —
430px에서는 대화만 보이고 목록이 숨는 것, 1280px에서는 **둘이 함께 보이고** 목록에서
바로 다른 방으로 옮겨지는 것, 열린 방 표시, `←`↔[나가기] 교체, 나간 뒤의 안내 문구.
콘솔 오류는 `/favicon.ico` 404 하나뿐이며 이 변경과 무관하다(파비콘이 원래 없다).
